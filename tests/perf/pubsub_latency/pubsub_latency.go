...
const (
	...
	serviceApplicationName = "perf-pubsub-latency"
)
...
func TestMain(m *testing.M) {
	utils.SetupLogs("pubsub_latency")
	...
	tr = runner.NewTestRunner("pubsublatency", testApps, nil, nil)
	os.Exit(tr.Start(m))
}

type PubsubMetrics struct {
	loadtest.K6RunnerMetricsSummary `json:",inline"`
	PublishFailures                 loadtest.K6CounterMetric `json:"publish_failures"`
}
...
func TestPubSubLatency(t *testing.T) {
	...
	summary, err := loadtest.K6Result[PubsubMetrics](k6Test)
	require.NoError(t, err)
